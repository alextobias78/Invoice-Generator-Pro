...
def create_pdf(invoice_data):
    ...
    # Payment Details
    elements.append(Spacer(1, 0.25*inch))
    if invoice_data['payment_type'] == 'iban':
        payment_info = f"""
        <b>Payment Details (IBAN):</b><br/>
        ...
        Bank: {invoice_data['bank_name']}<br/>
        Account Name: {invoice_data['account_name']}<br/>
        Routing Number: {invoice_data['routing_number']}<br/>
...
def create_invoice():
    if request.method == 'POST':
        ...
        invoice_data = {
            ...
            'bank_name': request.form['bank_name'] if request.form['payment_type'] == 'iban' else request.form['other_bank_name'],
            'account_name': request.form['account_name'] if request.form['payment_type'] == 'iban' else request.form['other_account_name'],
            'bic': request.form['bic'] if request.form['payment_type'] == 'iban' else None,
            'iban': request.form['iban'] if request.form['payment_type'] == 'iban' else None,
            'routing_number': request.form['routing_number'] if request.form['payment_type'] == 'other' else None,
            'account_number': request.form['account_number'] if request.form['payment_type'] == 'other' else None,
            'items': [],
            'subtotal': 0,
